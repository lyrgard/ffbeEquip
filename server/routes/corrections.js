...
route.post('/:server/corrections', async (req, res) => {
  console.log("TOTO");
    ...
    if (Object.keys(data).length > 10) {
        res.status(400).send("Only a maximum of 10 corrections can be send at a time");
        return;
    }
    for (var key in data) {
        if (isNaN(parseInt(key))) { 
            res.status(400).send(key + " is not valid as an item key");
            return;
        }
    ...
    if (Object.keys(tempItems).length > 2000) {
        res.status(500).send("The file is too big on the server");
    } else {
